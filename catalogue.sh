...
validate $? " downloading catalouge zip "
cd /app
validate $? " changing dir to /app "
unzip -o /tmp/catalogue.zip &>> $LOG_FILE
validate $? " unzip catalouge zip flie "
if [ $(pwd) == "/app" ]
then
...
validate $? " copy catalouge service file "
systemctl daemon-reload &>> $LOG_FILE
validate $? " catalouge deamon relaod "
systemctl enable catalogue &>> $LOG_FILE
validate $? " enable catalouge service "
systemctl start catalogue &>> $LOG_FILE
validate $? " starting catalouge service "
cp /home/centos/shell_script/mongo.repo /etc/yum.repos.d/mongo.repo
validate $? " copying mongo repo "
dnf install mongodb-org-shell -y  &>> $LOG_FILE
validate $? " install mongo db shell "
mongo --host mongodb.devops1234.online </app/schema/catalogue.js  &>> $LOG_FILE
